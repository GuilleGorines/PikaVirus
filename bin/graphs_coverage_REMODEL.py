...
def verify_detect_headers(valid_species_headers,
    ...
    for single_header in headers:
        for item in single_header:
            if item.lower() in valid_file_headers:
                file_column_index = single_header.index(item)

            elif item.lower() in valid_species_name_headers:  
                species_column_index = single_header.index(item)

            elif item.lower() in valid_subspecies_name_headers:
    ...
    try:
        ...
        if not species_column_index:
            print(f"The species name header was not detected or didnt have the proper name.\n \
                   Valid names for this header are: ${",".join(valid_species_name_headers)}.")
        if not subspecies_name_headers:
            print(f"The subspecies name header was not detected or didnt have the proper name.\n \
                   Valid names for this header are: ${",".join(valid_subspecies_name_headers)}.")
        if not file_headers:
            print(f"The subspecies name header was not detected or didnt have the proper name.\n \
                   Valid names for this header are: ${",".join(valid_file_headers)}.")
        print(f"Please consult the reference sheet format, sorry for the inconvenience!")
        sys.exit(1)

    return header_indexes

def remove_extensions(extension_list,
                      file_list,
                      index):
    """
    Given a list with extensions, and a level 1 nested list.
    Use the index to find the corresponding element on each list.
    Remove the said extensions from the string list, the nested list
...
species_data = [[line[header_indexes[0]],
                 line[header_indexes[1]], 
                 line[header_indexes[2]]] for line in species_data]

# Remove the extension of the file (so it matches the filename)
extensions = [".gz",".fna"]

species_data = remove_extension(extensions,
                                species_data,
                                index=2)
