...
def get_arguments():
    ...
    # parser.add_argument("", action="store_true", dest="create_html", default=False)
...
def main():
    ...
    # Full Boxplot: all genomes included
    full_boxplot = go.Figure()
    full_boxplot.update_layout(title_text = f"{args.sample_name}: all genomes depth distribution by single base",
                               yaxis_title = "Coverage Depth")
    full_boxplot_filename = ""
    
    # Full lineplot
    # Number of plots for mutliplot = number of entries in json
    full_lineplot = make_subplots(rows = len(plot_data), 
                                  cols = 1,
                                  y_title = "Proportion of bases above depth (%)",
                                  x_title = "Coverage Depth")
    full_lineplot_filename = ""

    # Parse JSON
    for assembly, data in plot_data.items():
        species = data["Species"]
        subspecies = data["Subspecies"]
        
        spp = f"{species} {subspecies}" if subspecies != "--" else species

        for fragment, plot_points in data["Sequences"].items():

            # Initialize boxplot figure
            single_boxplot = go.Figure()

            # Get the points for the Boxplot & flatten them
            single_boxplot_data = [[int(depth)]*int(quantity) for depth, quantity in plot_points["BoxPlot"].items()]
            single_boxplot_data = [ item for group in single_boxplot_data for item in group ]
            
            # Names of the boxplot (different if its genome or not)
            box_name    = "whole genome" if "genome" in fragment else fragment
            figure_name = f"{args.sample_name}: {spp} genome, depth distribution by single base" if "genome" in fragment else f"{args.sample_name}: {spp}; sequence: {fragment}, depth distribution by single base"
            file_name   = f"{args.sample_name}_{spp}_{assembly}_genome".replace(" ","_").replace("/","-") if "genome" in fragment else f"{args.sample_name}_{spp}_{assembly}_{fragment}".replace(" ","_").replace("/","-")

            # Generate the boxplot
            boxplot = go.Box(y = single_boxplot_data,
                             name = box_name,
                             boxmean = "sd")

            # Add boxplot to the data 
            single_boxplot.add_trace(boxplot)
            single_boxplot.update_layout(title_text = figure_name,
                                         yaxis_title = "Coverage Depth")

            plotly.offline.plot({"data": single_boxplot},
                                auto_open=False,
                                filename=f"{file_name}_single_boxplot.html")

            # Initialize lineplot
            single_lineplot = go.Figure()

            lineplot_data_x = [item for item in plot_points["LinePlot"].keys()]
            lineplot_data_y = [item for item in plot_points["LinePlot"].values()]
