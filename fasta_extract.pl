...
if(scalar @ARGV != 2){die "This scrtipt reads a multifasta and extracts only sequences whose IDs contain a string included in the IDs list file (non case-sensitive).\nThis script needs 2 arguments, first the multifasta file and second the list of IDs to find.\n\nUSAGE: perl fasta_extract.pl multifasta.fasta IDs.txt > output.fasta\n\n";}
